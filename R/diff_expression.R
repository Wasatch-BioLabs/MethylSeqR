                                  ...
                                  calc_type) 
{
  ...
  } else if (calc_type == "r_fisher") {
    cols <- c("c_counts_control", "c_counts_case", 
              "mod_counts_control", "mod_counts_case")
    pvals <- 
      pvals |>
      rowwise() |>
      mutate(
        p_val = fisher.test(matrix(c_across(cols), 2))$p.val)
  } else {
    stop("Invalid calc_type argument.")
  }
